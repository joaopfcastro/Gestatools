  ...
  return (
    <div className="flex flex-col h-full bg-transparent text-on-surface pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="flex justify-between items-center px-5 py-4 border-b border-surface-variant">
        <div className="inline-inline-flex items-center gap-1.5.5">
          <Icon name="history" className="text-primary icon-inline" />
        ...
        <div className="px-4 pt-4 pb-1 flex gap-2">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="ios-input w-full pl-10 h-12 md:h-11 text-base md:text-sm rounded-xl"
            />
          </div>
          <button
          ...
          >
            <Icon name="star" filled={showFavoritesOnly} className="icon-inline" />
          </button>
        </div>
      )}
      ...
      <div className="flex-grow overflow-y-auto p-4 flex flex-col gap-3">
        {records.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center h-64 text-secondary">
            <Icon name="person" className="text-secondary opacity-70 mb-2.5 text-[36px]" />
          ...
          <div className="flex flex-col items-center justify-center text-center h-48 text-secondary">
            <Icon name="search" className="text-secondary opacity-70 mb-2.5 text-[32px]" />
                ...
                <div className="bg-surface/50 border border-surface-variant/50 dark:bg-white/5 dark:border-white/5 rounded-xl p-4 flex justify-between items-start gap-3 transition-all hover:bg-surface-variant/50">
                  <div className="flex-grow flex flex-col gap-1.5">
                    <div className="inline-inline-flex items-center gap-1.5.5">
                      ...
                      <span className="text-xs text-secondary font-semibold inline-flex items-center gap-1.5">
                        <Icon name="calendar_today" className="text-[11px]" />
                        {formattedDate}
                      </span>
                    </div>
                    <h4 className="font-title-md text-sm font-bold text-on-surface truncate w-[180px] sm:w-[250px]">
                      {rec.patientName}
                    </h4>
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    ...
                    >
                      <Icon name="star" filled={rec.isFavorite} className="icon-inline" />
                    </button>
                    <button
                      onClick={() => {
                        triggerHaptic(100);
                    ...
                    >
                      <Icon name="delete" className="icon-inline" />
                    </button>
                  </div>
                </div>
              </SwipeToDelete>
